...
utils.experiment_context.bandwidth_value = 0.00003284380140738946
full_path = "../../enc-vpn-uncertainty-class-repl/processed_data/stable_cal_fraction/min_max_normalized/run0/frac_80"
params = HyperParameterStore().get_model_params("vpn_min_ece", "vpn_models")
...
rmd = Mahalanobis(z_support)  # Now compute mahalanobis # USE support to set mahal vars
...
m_k_rel = rmd.relative_mahalanobis_distance(z_cal).view(n_class, n_cal, -1).min(dim=0).values.view(n_class, n_cal) + 2
max_val = m_k_rel.max()
min_val = m_k_rel.min()
...
x_grid = np.linspace(1.85,max_val+0.025, 100000)
...
time2= time.time()
print(np.trapz(g_k[3].pdf(x_grid), x_grid, dx=0.0001))
